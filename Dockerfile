...
RUN echo "0 15 * * * root bash /app/src/scripts/run.sh konto >> /app/logs/cron.log 2>&1" > /etc/cron.d/puppeteer-cron && \
    #echo "0 16 * * 2,5 root bash /app/src/scripts/run.sh kk >> /app/logs/cron.log 2>&1" >> /etc/cron.d/puppeteer-cron && \
